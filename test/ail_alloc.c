...
void test(AIL_Allocator allocator, const char *name, bool lists_allocable, bool extendable)
{
	if (!lists_allocable) {
		if (test_alloc_single(&allocator, AIL_ALLOC_PAGE_SIZE/4))  printf("\033[032mSimple allocations      work for %s allocator :)\033[0m\n", name);
		else                                                       printf("\033[031mSimple allocations      fail for %s allocator :(\033[0m\n", name);
		if (test_calloc_single(&allocator, AIL_ALLOC_PAGE_SIZE/4)) printf("\033[032mSimple zero-allocations work for %s allocator :)\033[0m\n", name);
		else                                  			           printf("\033[031mSimple zero-allocations fail for %s allocator :(\033[0m\n", name);
		if (extendable) {
			if (test_alloc_single(&allocator, 2*AIL_ALLOC_PAGE_SIZE))  printf("\033[032mExtendable allocations      work for %s allocator :)\033[0m\n", name);
			else                                                       printf("\033[031mExtendable allocations      fail for %s allocator :(\033[0m\n", name);
			if (test_calloc_single(&allocator, 2*AIL_ALLOC_PAGE_SIZE)) printf("\033[032mExtendable zero-allocations work for %s allocator :)\033[0m\n", name);
		...
		if (extendable) {
			...
			if (test_calloc(&allocator, 2*AIL_ALLOC_PAGE_SIZE))  printf("\033[032mExtendable zero-allocations work for %s allocator :)\033[0m\n", name);
			else                                  			     printf("\033[031mExtendable zero-allocations fail for %s allocator :(\033[0m\n", name);
			if (test_realloc(&allocator, 2*AIL_ALLOC_PAGE_SIZE)) printf("\033[032mExtendable re-allocations   work for %s allocator :)\033[0m\n", name);
			else                                                 printf("\033[031mExtendable re-allocations   fail for %s allocator :(\033[0m\n", name);
		}
	}
}
